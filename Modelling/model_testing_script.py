...
results.to_csv("../MIND_large/csv/als_testing_output.csv")
    ...
    for k in [5, 25, 50, 100]:
        ...
        for lambda_reg in [0.1, 1, 5]: 
            ...
            for max_iteration in [100]:
                ...
                U, V, track_error, track_update = matrix_modules.vectorized_gradient_descent(R, U, V, D, rate=0.00001, max_iterations=max_iteration, lam=lambda_reg)
                param_info = pd.DataFrame(data={"clustering_type": clustering, "alg" : "SGD", "k" : k, "lambda_reg" : lambda_reg, 'added_features' : i, 'RMSE' : [track_error], 'Max Updates' : [track_update]}, index=[counter])
                counter += 1
                results = pd.concat([results, param_info], axis=0)

results.to_csv("../MIND_large/csv/sgd_testing_output.csv")